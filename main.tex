...
To build the transaction, two scans of the dictionary $records$ are needed: the first one to create a node for each record, the second one to assign the degrees of kinship between nodes. This process is described in the Algorithm~\ref{build_trees}, which returns a list of trees, containing the root node of each transaction.
...
\For{$rid \in records$}
	...
	\Else
		...
		\State $parent._{children} \gets parent._{children} \cup \{child\}$
		\State $child._{parent} \gets parent$
	\EndIf
\EndFor
\Return $roots$
\EndFunction
